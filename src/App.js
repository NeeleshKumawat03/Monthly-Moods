...
class App extends Component {
  state = {
    activeMonth: initialMonthsList[0],
  ...
  setMood = index => {
    const {activeMonth, selectedEmoji} = this.state
    let day
    switch (activeMonth.dates[index].date % 7) {
      case 1:
        day = 'Sun'
        break
      case 2:
    ...
    activeMonth.dates[index].emojiUrl = selectedEmoji.emojiUrl
    activeMonth.dates[index].emojiName = selectedEmoji.emojiName
    activeMonth.dates[index].day = day
    const {dates} = activeMonth
    this.setState({
      activeMonth: {...activeMonth, dates},
    })
    console.log(activeMonth)
  }

  setActiveNextMonth = month => {
    month = month === 12 ? 0 : month
    this.setState({
      activeMonth: initialMonthsList[month],
    })
  }

  setActivePrevMonth = month => {
  ...
  setSelectedEmojiName = emojiName => {
    this.setState({
    ...
    return (
      <DailyMoodsContext.Provider
        value={{
          ...
          setSelectedEmojiName: this.setSelectedEmojiName,
          setActivePrevMonth: this.setActivePrevMonth,
          setActiveNextMonth: this.setActiveNextMonth,
          setMood: this.setMood,
          setSelectEmoji: this.setSelectEmoji,
        }}
      >
